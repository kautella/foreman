...
| P2-004 | Complete — implement the Codex CLI adapter | Implemented portable Codex executable and authentication diagnosis, exact profile-to-launch-spec resolution, task-type sandbox selection, and bounded JSONL plus structured final-output collection. The Phase 2 research path now also has one real successful run. The manifest remains `implemented-unverified` because resume and interrupt are outside this slice. |
| P2-005 | Complete — implement the tmux adapter | Implemented portable tmux diagnosis, exact endpoint reservation, validated-spec launch through an owned runner, liveness inspection, 64 KiB capture, and proven-owned session closure. The real lifecycle passed on tmux `3.7c` through an isolated adapter-owned server. The broader manifest remains `implemented-unverified` because later operations are outside this slice. |
...
| P2-009 | Complete — establish verification evidence | The portable fakes and disposable Git fixtures cover isolation, locks, recovery, validation, handoff, refusal, and the opt-in live-harness contract. A real tmux lifecycle passed with a harmless fake worker and isolated adapter-owned server. The first authorized Codex attempt safely exposed an unsupported `uniqueItems` output-schema constraint; after its compatible-schema fix, the second authorized read-only Codex research task completed with durable structured output, validation, report, clean exact-base repository, and exact-task teardown evidence. |
| P2-010 | Complete — publish Phase 2 readiness evidence | Generated `~/.foreman/projects/foreman/reports/report-phase-2-readiness.html`, a dark standalone report covering the completed local-only slice, its evidence, the safely handled live-environment findings, and deferred scope. |
