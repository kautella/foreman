...
| P2-006 | Launch and reconcile approved tasks | Add the neutral task command surface for starting, observing, and reconciling one approved task. Refuse unapproved plans, unavailable adapters, mismatched identities, or ambiguous state. |
| P2-007 | Validate and prepare local handoffs | Run declared validation in the task environment, record command, output, exit status, and exact Git state. Produce a local change handoff or dark research report without pushing, publishing, merging, or landing. |
| P2-008 | Recover and tear down safely | Reconcile interrupted tasks from durable state, Git, tmux, and Codex evidence. Preserve uncertain work. Teardown requires confirmed local landing or explicit discard authority for the exact task-owned worktree. |
| P2-009 | Establish verification evidence | Add portable fakes for Codex and tmux plus disposable Git fixtures for isolation, lock, recovery, validation, handoff, and refusal paths. Add opt-in live checks for tmux and one real Codex task before claiming the reference path is supported. |
