...
| P2-009 | Active — establish verification evidence | The portable fakes and disposable Git fixtures now cover isolation, locks, recovery, validation, handoff, refusal, and the opt-in live-harness contract. `scripts/live-check.sh` adds durable caller-selected evidence for a real tmux lifecycle with a harmless fake worker and a separately explicit real Codex research task. The real tmux check passed on 17 September 2026 using an isolated adapter-owned server; its evidence is retained under the Foreman project. The first authorized Codex attempt confirmed authentication but stopped safely when Codex rejected the output schema's unsupported `uniqueItems` constraint. A revised schema and portable compatibility check are ready; one newly authorized real Codex run must confirm the complete path. |
