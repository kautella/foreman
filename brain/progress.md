...
- The harness contract has a dedicated portable test, bringing the full suite to fifteen portable test files. The real tmux check initially exposed a macOS Unix-socket path-limit failure; the harness now chooses a unique short adapter-owned server name and the corrected check passed. The maintainer reports Codex CLI `0.147.0` is now authenticated. No real worker has yet launched and no account usage has been consumed. P2-009 remains active until the maintainer explicitly authorizes the one real task and its durable evidence is reviewed.
