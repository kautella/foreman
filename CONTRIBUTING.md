...
Read [AGENTS.md](AGENTS.md) and the relevant files in [brain/](brain/README.md) before beginning substantial work.
...
The checks reject whitespace errors and tracked local IDE metadata, require the core project documents, validate active JSON with `jq`, verify Bash syntax, run the portable test suite, and enforce the active-code terminology, provider, instruction, and external-source boundaries.

Tests that can remove or rewrite a repository must run only inside a purpose-built expendable environment with exact ownership markers and fail-closed cleanup. Ordinary Foreman tests use isolated temporary fixtures and must not depend on personal configuration, credentials, or a hosted Git service.

When adding or changing a JSON contract, update its machine-readable schema, runtime validation, representative examples, malformed-input tests, and user-facing diagnostics together. Adapter manifests may describe only implemented or explicitly contract-only operations. Provider executable calls belong inside the applicable adapter, never in the core.
